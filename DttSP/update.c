...
unsigned int threadno = 2;	// yt7pwr
...
//	sem_wait(&top[2].sync.upd.sem);
	if ((setit > 0) && (setit<2)) threadno = setit;
...
DttSP_EXP int SetMode (unsigned int thread, unsigned int subrx, SDRMODE m)		// yt7pwr
{
	int rtn=0;
	sem_wait(&top[thread].sync.upd.sem);
	...
	if(m == WFM)
		rx[thread][subrx].fm.gen->wide_enabled = 1;
	else
		rx[thread][subrx].fm.gen->wide_enabled = 0;

	sem_post(&top[thread].sync.upd.sem);
	return rtn;
}

DttSP_EXP int SetTXMode (unsigned int thread, SDRMODE m)							// yt7pwr
{
	int rtn=0;
	sem_wait(&top[thread].sync.upd.sem);
	uni[thread].mode.sdr = m; 
	tx[thread].mode = m;
...
DttSP_EXP int FMreload (unsigned int thread, unsigned int subrx, double low_frequency, double high_frequency,
						double bandwith, int wide)
{
	sem_wait(&top[thread].sync.upd.sem);
	FMDreload(rx[thread]->fm.gen, low_frequency, high_frequency, bandwith, wide);
	sem_post(&top[thread].sync.upd.sem);
}

DttSP_EXP int FMenableStereo (unsigned int thread, unsigned int subrx, int stereo)
{
	sem_wait(&top[thread].sync.upd.sem);
	rx[thread]->fm.gen->stereo_decoder->StereoEnabled = stereo;
	sem_post(&top[thread].sync.upd.sem);
}
...
DttSP_EXP int
SetRXHighPassFilter (unsigned int thread, unsigned int subrx, double cut_frequency)		// yt7pwr
{
	int ncoef = uni[thread].buflen + 1;
	int i, fftlen = 2 * uni[thread].buflen,rtn=0;
	fftwf_plan ptmp;
	...
	if (fabs (cut_frequency) >= 0.5 * uni[thread].samplerate)
		rtn = -1;

	if (rtn == 0)
	{
		sem_wait(&top[thread].sync.upd.sem);
		delFIR_COMPLEX (rx[thread][subrx].high_pass_filt.coef);

		rx[thread][subrx].high_pass_filt.coef = newFIR_Highpass_COMPLEX ((REAL)cut_frequency,
			uni[thread].samplerate, ncoef);
		zcvec = newvec_COMPLEX (fftlen, "filter2 z vec in setFilter");

		ptmp = fftwf_plan_dft_1d (fftlen,
			(fftwf_complex *) zcvec,
			(fftwf_complex *) rx[thread][subrx].high_pass_filt.ovsv->zfvec,
			FFTW_FORWARD, uni[thread].wisdom.bits);

#ifdef LHS
		for (i = 0; i < ncoef; i++)
			zcvec[i] = rx[thread][subrx].high_pass_filt.coef->coef[i];
#else
		for (i = 0; i < ncoef; i++)
			zcvec[fftlen - ncoef + i] = rx[thread][subrx].high_pass_filt.coef->coef[i];
#endif
		fftwf_execute (ptmp);
		fftwf_destroy_plan (ptmp);
		delvec_COMPLEX (zcvec);
		normalize_vec_COMPLEX (rx[thread][subrx].high_pass_filt.ovsv->zfvec, rx[thread][subrx].high_pass_filt.ovsv->fftlen,
			rx[thread][subrx].high_pass_filt.ovsv->scale);

		memcpy ((char *) rx[thread][subrx].high_pass_filt.save, (char *) rx[thread][subrx].high_pass_filt.ovsv->zfvec,
			rx[thread][subrx].high_pass_filt.ovsv->fftlen * sizeof (COMPLEX));
		sem_post(&top[thread].sync.upd.sem);
	}
	return rtn;
}
...
DttSP_EXP int
SetRXLowPassFilter (unsigned int thread, unsigned int subrx, double cut_frequency)		// yt7pwr
{
	int ncoef = uni[thread].buflen + 1;
	int i, fftlen = 2 * uni[thread].buflen,rtn=0;
	fftwf_plan ptmp;
	...
	{
		sem_wait(&top[thread].sync.upd.sem);
		delFIR_COMPLEX (rx[thread][subrx].low_pass_filt.coef);

		rx[thread][subrx].low_pass_filt.coef = newFIR_Lowpass_COMPLEX ((REAL)cut_frequency,
			uni[thread].samplerate, ncoef);
		zcvec = newvec_COMPLEX (fftlen, "filter2 z vec in setFilter");

		ptmp = fftwf_plan_dft_1d (fftlen,
			(fftwf_complex *) zcvec,
			(fftwf_complex *) rx[thread][subrx].low_pass_filt.ovsv->zfvec,
			FFTW_FORWARD, uni[thread].wisdom.bits);
...
#else
		for (i = 0; i < ncoef; i++)
			zcvec[fftlen - ncoef + i] = rx[thread][subrx].low_pass_filt.coef->coef[i];
#endif
		fftwf_execute (ptmp);
		fftwf_destroy_plan (ptmp);
		delvec_COMPLEX (zcvec);
		normalize_vec_COMPLEX (rx[thread][subrx].low_pass_filt.ovsv->zfvec, rx[thread][subrx].low_pass_filt.ovsv->fftlen,
			rx[thread][subrx].low_pass_filt.ovsv->scale);

		memcpy ((char *) rx[thread][subrx].low_pass_filt.save, (char *) rx[thread][subrx].low_pass_filt.ovsv->zfvec,
			rx[thread][subrx].low_pass_filt.ovsv->fftlen * sizeof (COMPLEX));
		sem_post(&top[thread].sync.upd.sem);
	}
	return rtn;
 }
...
DttSP_EXP void SetRXOscPhase(double phase)
{
	int i,j;
	sem_wait(&top[0].sync.upd.sem);
	for(j=0;j<uni[0].multirx.nrx;j++) rx[0][j].osc.phase = phase;
	sem_post(&top[0].sync.upd.sem);
}
...
DttSP_EXP int SetSampleRate (unsigned int thread, double newSampleRate)
{
	extern int reset_for_samplerate (unsigned int, REAL);
	int rtn = -1;
	...
	{
		if (reset_for_samplerate (thread, samplerate) != -1)
			rtn = 0;
	}
...
DttSP_EXP void SetANF (unsigned int thread, unsigned subrx, BOOLEAN setit)
{
	sem_wait(&top[thread].sync.upd.sem);
...
DttSP_EXP void SetNB (unsigned int thread, unsigned subrx, BOOLEAN setit)
{
	sem_wait(&top[thread].sync.upd.sem);
...
DttSP_EXP void SetNBvals (unsigned int thread, unsigned subrx, double threshold)
{
	sem_wait(&top[thread].sync.upd.sem);
...
DttSP_EXP void SetRXAGC (unsigned int thread, unsigned subrx, AGCMODE setit)
{
	sem_wait(&top[thread].sync.upd.sem);
...
DttSP_EXP void SetRXAGCAttack (unsigned int thread, unsigned subrx, int attack)
{
...
DttSP_EXP void
ResetRingBuffer (unsigned int thread)
{
	extern int IQdoit;
	extern BOOLEAN reset_em;
...
DttSP_EXP void
SetIQSuspended(int setit)		// yt7pwr
{
	extern BOOLEAN IQsuspended;
	sem_wait(&top[0].sync.upd.sem);
	IQsuspended = setit;
	sem_post(&top[0].sync.upd.sem);
}
...
DttSP_EXP void
SetIQFixed(int thread, int subrx, int setit, REAL gain, REAL phase)		// yt7pwr
{
	extern BOOLEAN IQfixed;
	sem_wait(&top[0].sync.upd.sem);
	IQfixed = setit;
	rx[thread][subrx].iqfix->RXgain = (REAL) (1.0 + 0.001 * gain);
	rx[thread][subrx].iqfix->RXphase = (REAL) (0.001 * phase);
	sem_post(&top[0].sync.upd.sem);
}
...
SetCorrectRXIQw (unsigned int thread, unsigned int subrx, REAL wr, REAL wi, unsigned int index)
{
	...
	if(index == 100)
			rx[thread][subrx].iqfix->b[0] = Cmplx((REAL)wr, (REAL)wi);	// yt7pwr
	else
		rx[thread][subrx].iqfix->w[index] = Cmplx((REAL)wr,(REAL)wi);

	if (index == 0)
	{
		memset((void *)&rx[thread][subrx].iqfix->w[1],0,15*sizeof(COMPLEX));
		memset((void *)&rx[thread][subrx].iqfix->b[0],0,15*sizeof(COMPLEX));
	}
	sem_post(&top[thread].sync.upd.sem);
}
...
SetWindow (unsigned int thread, Windowtype window)
{
	...
	if (!uni[thread].spec.polyphase)
	{
		...
		makewindow (window, uni[thread].spec_scope.size, uni[thread].spec_scope.window);	// yt7pwr
	}
	uni[thread].spec.wintype = window;
	uni[thread].spec_scope.wintype = window;		// yt7pwr
	sem_post(&top[thread].sync.upd.sem);
}
...
DttSP_EXP void
SetRXFilterMode (unsigned int thread, unsigned subrx, FILTERMODE mode)			// yt7pwr
{
	sem_wait(&top[thread].sync.upd.sem);
	rx[thread][subrx].filter_mode = mode;
	sem_post(&top[thread].sync.upd.sem);
}
...
DttSP_EXP
void SetSquelchState (unsigned int thread, unsigned int subrx,BOOLEAN setit)
{
	sem_wait(&top[thread].sync.upd.sem);
...
int DEFSPEC = 4096;
DttSP_EXP void SetSPEClen (unsigned int thread, int newSize)
{
	DEFSPEC = newSize;
}
...
DttSP_EXP void SetAudioSize (unsigned int thread, unsigned int size)
{
		top[thread].susp = TRUE;
		sem_wait (&top[thread].sync.upd.sem);
		...
		ringb_float_reset (top[thread].jack.ring.o.l);
		ringb_float_reset (top[thread].jack.ring.o.r);
		...
		ringb_float_restart (top[thread].jack.ring.i.l, top[thread].hold.size.frames);
		ringb_float_restart (top[thread].jack.ring.i.r, top[thread].hold.size.frames);
...
Process_Scope (unsigned int thread, float *results, int numpoints)
{
	...
	//sem_wait (&top[thread].sync.upd.sem);
	uni[thread].spec_scope.type = SPEC_POST_AGC;
	uni[thread].spec_scope.scale = SPEC_PWR;
	uni[thread].spec_scope.rxk = 0;

	snap_scope (&uni[thread].spec_scope, uni[thread].spec_scope.type);
	//sem_post (&top[thread].sync.upd.sem);
	for (i = 0; i < numpoints; i++)
	{
		results[i] = (float) CXBreal (uni[thread].spec_scope.timebuf, i);
	}
}
...
DttSP_EXP void *NewResampler (int samplerate_in, int samplerate_out)
{
	ResSt tmp;
...
DttSP_EXP void DoResampler (COMPLEX * input, COMPLEX * output, int numsamps, int *outsamps,
             ResSt ptr)
{
	ptr->input = input;
	ptr->output = output;
...
DttSP_EXP void DelPolyPhaseFIR (ResSt resst)
{
...
{
	ResStF tmp;
	int lcm = 61440000; /*28224000*/
	int interpFactor, deciFactor;
	interpFactor = (int)(lcm / samplerate_in);
	deciFactor = (int)(lcm / samplerate_out);
	tmp = newPolyPhaseFIRF (4096, 0, interpFactor, 0, deciFactor);
	return (void *) tmp;
}

DttSP_EXP void *NewResamplerF_LimeSDR (int lcm, int samplerate_in, int samplerate_out)
{
	ResStF tmp;
	int interpFactor, deciFactor;
	interpFactor = (int)(lcm / samplerate_in);
...
DttSP_EXP void DoResamplerF (float *input, float *output, int numsamps, int *outsamps,
              ResStF ptr)
{
	ptr->input = input;
	ptr->output = output;
...
DttSP_EXP void SetResamplerF (unsigned int thread, unsigned int subrx, int samplerate_in,
					int samplerate_out)		// yt7pwr
{
	/*ResStF tmp_l, tmp_r;
	int lcm = 28224000, interpFactor, deciFactor;
	sem_wait(&top[thread].sync.upd.sem);
	DelPolyPhaseFIRF(rx[thread][subrx].resample.gen2i);
	DelPolyPhaseFIRF(rx[thread][subrx].resample.gen2r);
	interpFactor = lcm / samplerate_in;
	deciFactor = lcm / samplerate_out;
	tmp_l = newPolyPhaseFIRF (4096, 0, interpFactor, 0, deciFactor);
	tmp_r = newPolyPhaseFIRF (4096, 0, interpFactor, 0, deciFactor);
	rx[thread][subrx].resample.gen2i = (void *) tmp_l;
	rx[thread][subrx].resample.gen2r = (void *) tmp_r;
	sem_post(&top[thread].sync.upd.sem);*/
	ResStF tmp;
	int lcm = 28224000, interpFactor, deciFactor;
	interpFactor = lcm / samplerate_in;
...
DttSP_EXP void EnableResamplerF (unsigned int thread, unsigned int subrx, int doit)		// yt7pwr
{
	sem_wait(&top[thread].sync.upd.sem);
	rx[thread][subrx].resample.flag = doit;
	sem_post(&top[thread].sync.upd.sem);
}
...
DttSP_EXP int
SetCTCSSOscFreq (unsigned int thread, double newfreq)						// yt7pwr
{
	if (fabs (newfreq) >= 0.5 * uni[thread].samplerate)
		return -1;
	...
	newfreq *= 2.0 * M_PI / uni[thread].samplerate;
	sem_wait(&top[thread].sync.upd.sem);
	tx[thread].ctcss.osc->Frequency = (REAL)newfreq;
	sem_post(&top[thread].sync.upd.sem);
	return 0;
}

DttSP_EXP int
SetCTCSSOscAmplitude (unsigned int thread, double newamplitude)			// yt7pwr
{
	sem_wait(&top[thread].sync.upd.sem);
	tx[thread].ctcss.amplitude = (REAL)newamplitude;
	sem_post(&top[thread].sync.upd.sem);
	return 0;
}

DttSP_EXP void
EnableCTCSS (unsigned int thread, BOOLEAN setit)						// yt7pwr
{
	sem_wait(&top[thread].sync.upd.sem);
	tx[thread].ctcss.flag = setit;
	sem_post(&top[thread].sync.upd.sem);
}