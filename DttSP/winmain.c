...
//		ringb_float_reset (top[thread].jack.auxr.i.r);
		...
		ringb_float_clear (top[thread].jack.ring.i.l, top[thread].jack.size * loc[thread].mult.ring-1);
		ringb_float_clear (top[thread].jack.ring.o.l, top[thread].jack.size * loc[thread].mult.ring-1);
		ringb_float_clear (top[thread].jack.ring.o.l, top[thread].jack.size * loc[thread].mult.ring-1);
		ringb_float_reset (top[thread].jack.ring.i.l);
		ringb_float_reset (top[thread].jack.ring.i.r);
//		ringb_float_reset (top[thread].jack.auxr.i.l);
//		ringb_float_reset (top[thread].jack.auxr.i.r);
		...
		ringb_float_restart (top[thread].jack.ring.i.r, reset_size);
		ringb_float_restart (top[thread].jack.ring.i.l, reset_size);

		ringb_float_restart (top[thread].jack.auxr.i.r, reset_size);
		ringb_float_restart (top[thread].jack.auxr.i.l, reset_size);
...
DttSP_EXP void
Audio_Callback (unsigned int thread, float *input_l, float *input_r, float *output_l,
...
DttSP_EXP void
Audio_Input_Callback (unsigned int thread,float *input_l, float *input_r, unsigned int nframes)		// yt7pwr
{
	BOOLEAN b = reset_em;
	int i = thread;
...
DttSP_EXP void
Audio_Output_Callback (unsigned int thread, float *output_l, float *output_r, unsigned int nframes)		// yt7pwr
{
	BOOLEAN b = reset_em;
	int i = thread;
	if (top[i].susp)
...
DttSP_EXP void
Audio_Callback2 (unsigned int thread_no, float **input, float **output, unsigned int nframes)
{
	unsigned int thread;
	BOOLEAN b = reset_em;
...
DttSP_EXP void process_TX_samples_thread (unsigned int tx_proc_thread)
{
	while (top[tx_proc_thread].running)
	{
		sem_wait (&top[tx_proc_thread].sync.tx.sem);

		switch (top[tx_proc_thread].state)
		{
			case RUN_MUTE:
				run_mute (tx_proc_thread);
				break;
			case RUN_PASS:
				run_pass (tx_proc_thread);
				break;
			case RUN_PLAY:
				run_play (tx_proc_thread);
				break;
			case RUN_SWCH:
				run_swch (tx_proc_thread);
				break;
		}
	}
}
...
void closeup ()
{
	...
	for(thread = 0; thread<threadno;thread++)	// yt7pwr
	{
...
//	top[thread].jack.size = 2048;
	top[thread].jack.size = 65536; //top[thread].hold.size.frames;
	...
	ringb_float_clear (top[thread].jack.auxr.i.l, top[thread].hold.size.frames);
	ringb_float_clear (top[thread].jack.auxr.i.r, top[thread].hold.size.frames);
...
setup_threading (unsigned int thread)
{
	...
	sem_init (&top[thread].sync.tx.sem, 0, 0);	
}
...
PRIVATE void setup_defaults (unsigned int thread)
{
	...
	if(thread == 0)
		...
		loc[thread].def.spec = DEFSPEC_TX;
...
int reset_for_samplerate (unsigned int thread, REAL new_samplerate)
{	
		safefree ((char *) top[thread].hold.buf.r);
		safefree ((char *) top[thread].hold.buf.l);
		safefree ((char *) top[thread].hold.aux.r);
