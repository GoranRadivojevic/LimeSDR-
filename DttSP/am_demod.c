...
INLINE BOOLEAN isNaN( REAL r )		// yt7pwr
{
   return r != r;
}
	...
	switch (am->mode)
	{
		...
		case AMdet:
			for (i = 0; i < am->size; i++)
			{
				...
				if(am->lock.curr > 1000 || isNaN(am->lock.curr))		// yt7pwr
					am->lock.curr = 1000;

				am->dc = 0.9999f * am->dc + 0.0001f * am->lock.curr;

				if(am->dc > 1000 || isNaN(am->dc) )				// yt7pwr
					am->dc = 1000;

				am->smooth = 0.5f * am->smooth + 0.5f * (am->lock.curr - am->dc);

				if(am->smooth > 1000 || isNaN(am->smooth))			// yt7pwr
					am->smooth = 1000;
