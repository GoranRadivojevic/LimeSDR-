...
{
  COMPLEX tmp;
  int i,j,k, m = pflt->fftlen, n = pflt->buflen;
...
void
filter_CTCSS(FiltOvSv pflt)							// yt7pwr
{
  ...
  {
	zivec[i] = Cmul(zivec[i], zfvec[i]);
	//zivec[i] = Cmul(zivec[i], OSCCdata(osc, i));
  }
...
newFiltOvSv(COMPLEX * coefs, int ncoef, int pbits) 
{
  ...
  COMPLEX *zrvec, *zfvec, *zivec, *zovec, *coef;
  ...
  coef = newvec_COMPLEX_16(fftlen, "coeficients");
