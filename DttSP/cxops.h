...
static INLINE COMPLEX
CaddA (COMPLEX x, COMPLEX y, REAL ampl)
{
	COMPLEX z;
	c_re (z) = c_re (x) + c_re (y) * ampl;
	c_im (z) = c_im (x) + c_im (y) * ampl;
	return z;
}
...
static INLINE REAL
Cmodulus (COMPLEX z)	// yt7pwr
{
	return (REAL) sqrt (z.re*z.re + z.im*z.im);
}

static INLINE REAL
Cargument (COMPLEX z)	// yt7pwr
{
	return (REAL) atan2(z.im, z.re);
}
...
static INLINE COMPLEX
CmplxA (REAL x, IMAG y, REAL ampl)
{
	COMPLEX z;
	z.re = x, z.im = y * ampl;
	return z;
