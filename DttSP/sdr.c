...
reset_spectrum (unsigned int thread)
{
	...
	if (uni[thread].spec_scope.flag)				// yt7pwr
		reinit_spectrum (&uni[thread].spec_scope);
}
	...
	int specsize, int numrecv, int cpdsize, unsigned int thread)
{
	...
	uni[thread].spec_scope.rxk = 0;							// yt7pwr
	uni[thread].spec_scope.buflen = uni[thread].buflen;
	uni[thread].spec_scope.scale = SPEC_PWR;
	uni[thread].spec_scope.type = SPEC_POST_FILT;
	uni[thread].spec_scope.size = specsize;
	uni[thread].spec_scope.planbits = uni[thread].wisdom.bits;
	init_spectrum (&uni[thread].spec_scope);
...
//////////////////////////////// high pass ////////////////////////////////////////////////////    yt7pwr
	rx[thread][k].high_pass_filt.coef = newFIR_Highpass_COMPLEX (50.0,	uni[thread].samplerate, uni[thread].buflen + 1);
	rx[thread][k].high_pass_filt.ovsv =
		newFiltOvSv (FIRcoef (rx[thread][k].high_pass_filt.coef), FIRsize (rx[thread][k].high_pass_filt.coef),
		uni[thread].wisdom.bits);
	normalize_vec_COMPLEX (rx[thread][k].high_pass_filt.ovsv->zfvec, rx[thread][k].high_pass_filt.ovsv->fftlen,
		rx[thread][k].high_pass_filt.ovsv->scale);

	rx[thread][k].high_pass_buf.i = newCXB (FiltOvSv_fetchsize (rx[thread][k].high_pass_filt.ovsv),
		FiltOvSv_fetchpoint (rx[thread][k].high_pass_filt.ovsv),
		"init rx[thread][k].high_pass_buf.i");
	rx[thread][k].high_pass_buf.o = newCXB (FiltOvSv_storesize (rx[thread][k].high_pass_filt.ovsv),
		FiltOvSv_storepoint (rx[thread][k].high_pass_filt.ovsv),
		"init rx[thread][k].high_pass_buf.o");

	rx[thread][k].high_pass_filt.save =
		newvec_COMPLEX (rx[thread][k].high_pass_filt.ovsv->fftlen, "RX high_pass_filter cache");
	memcpy ((char *) rx[thread][k].high_pass_filt.save, (char *) rx[thread][k].high_pass_filt.ovsv->zfvec,
		rx[thread][k].high_pass_filt.ovsv->fftlen * sizeof (COMPLEX));

//////////////////////////////// low pass /////////////////////////////////////////////////////    yt7pwr
	rx[thread][k].low_pass_filt.coef = newFIR_Lowpass_COMPLEX (550.0, uni[thread].samplerate, uni[thread].buflen + 1);
	rx[thread][k].low_pass_filt.ovsv =
		newFiltOvSv (FIRcoef (rx[thread][k].low_pass_filt.coef), FIRsize (rx[thread][k].low_pass_filt.coef),
		uni[thread].wisdom.bits);
	normalize_vec_COMPLEX (rx[thread][k].low_pass_filt.ovsv->zfvec, rx[thread][k].low_pass_filt.ovsv->fftlen,
		rx[thread][k].low_pass_filt.ovsv->scale);

	rx[thread][k].low_pass_buf.i = newCXB (FiltOvSv_fetchsize (rx[thread][k].low_pass_filt.ovsv),
		FiltOvSv_fetchpoint (rx[thread][k].low_pass_filt.ovsv),
		"init rx[thread][k].low_pass_buf.i");
	rx[thread][k].low_pass_buf.o = newCXB (FiltOvSv_storesize (rx[thread][k].low_pass_filt.ovsv),
		FiltOvSv_storepoint (rx[thread][k].low_pass_filt.ovsv),
		"init rx[thread][k].low_pass_buf.o");

	rx[thread][k].low_pass_filt.save =
		newvec_COMPLEX (rx[thread][k].low_pass_filt.ovsv->fftlen, "RX low_pass_filter cache");
	memcpy ((char *) rx[thread][k].low_pass_filt.save, (char *) rx[thread][k].low_pass_filt.ovsv->zfvec,
		rx[thread][k].low_pass_filt.ovsv->fftlen * sizeof (COMPLEX));
///////////////////////////////////////////////////////////////////////////////////////////////

	rx[thread][k].filter_mode = PASS_BAND;
...

	{
		...
		interpFactor = lcm / uni[thread].samplerate;
		deciFactor = lcm / uni[thread].samplerate;
		tmp_i = newPolyPhaseFIRF (4096, 0, interpFactor, 0, deciFactor);
		tmp_r = newPolyPhaseFIRF (4096, 0, interpFactor, 0, deciFactor);
		rx[thread][k].resample.gen2i = (void *) tmp_i;
		rx[thread][k].resample.gen2r = (void *) tmp_r;
	}
}
...
{
	/* conditioning */
	tx[thread].iqfix = newCorrectIQ (0.0, 1.0, 0.000f);
	...
	/*ctcss filter*/
	tx[thread].ctcss_filt.coef = newFIR_Bandpass_COMPLEX (50.0, 3000.0,
		uni[thread].samplerate, uni[thread].buflen + 1);
	tx[thread].ctcss_filt.ovsv = newFiltOvSv (FIRcoef (tx[thread].ctcss_filt.coef),
		FIRsize (tx[thread].ctcss_filt.coef), uni[thread].wisdom.bits);
	tx[thread].ctcss_filt.ovsv_pre = newFiltOvSv (FIRcoef (tx[thread].ctcss_filt.coef),
		FIRsize (tx[thread].ctcss_filt.coef), uni[thread].wisdom.bits);
	normalize_vec_COMPLEX (tx[thread].ctcss_filt.ovsv->zfvec, tx[thread].ctcss_filt.ovsv->fftlen,tx[thread].ctcss_filt.ovsv->scale);

	tx[thread].ctcss_filt.save = newvec_COMPLEX (tx[thread].ctcss_filt.ovsv->fftlen, "CTCSS filter cache");
	memcpy ((char *) tx[thread].ctcss_filt.save,
		(char *) tx[thread].ctcss_filt.ovsv->zfvec,
		tx[thread].ctcss_filt.ovsv->fftlen * sizeof (COMPLEX));

	tx[thread].ctcss_buf.i = newCXB (FiltOvSv_fetchsize (tx[thread].ctcss_filt.ovsv),
		FiltOvSv_fetchpoint (tx[thread].ctcss_filt.ovsv), "init tx[thread].ctcss_buf.i");
	tx[thread].ctcss_buf.o = newCXB (FiltOvSv_storesize (tx[thread].ctcss_filt.ovsv),
		FiltOvSv_storepoint (tx[thread].ctcss_filt.ovsv), "init tx[thread].ctcss_buf.o");
	...
	tx[thread].ctcss.osc = newOSC (uni[thread].buflen,
		ComplexTone,
		123.0, 0.0, uni[thread].samplerate, "CTCSS Oscillator");		// 123Hz
}
...
destroy_workspace (unsigned int thread)
{
	...
	delCXB (tx[thread].ctcss_buf.i);
	delCXB (tx[thread].ctcss_buf.o);
	delOSC(tx[thread].ctcss.osc);
	...
	for (k = 0; k < uni[thread].multirx.nrx; k++)
	{
		...
		delFiltOvSv (rx[thread][k].high_pass_filt.ovsv);					// yt7pwr
		delFiltOvSv (rx[thread][k].low_pass_filt.ovsv);						// yt7pwr
		...
		delPolyPhaseFIRF(rx[thread][k].resample.gen2i);
		delPolyPhaseFIRF(rx[thread][k].resample.gen2r);
	}

	/* all */
	finish_spectrum (&uni[thread].spec);
	finish_spectrum (&uni[thread].spec_scope);								// yt7pwr
...
PRIVATE void
do_rx_scope_spectrum (int k, unsigned int thread, CXB buf, int type)		// yt7pwr
{
	memcpy ((char *) &CXBdata (uni[thread].spec_scope.accum, uni[thread].spec_scope.fill),
		(char *) CXBbase (buf), CXBsize (buf) * sizeof (COMPLEX));

	uni[thread].spec_scope.fill = (uni[thread].spec_scope.fill + CXBsize (buf)) & uni[thread].spec_scope.mask;
}
...
do_tx_spectrum (unsigned int thread, CXB buf)
{
	...
	else
	{
		...
		memcpy ((char *) &CXBdata (uni[thread].spec_scope.accum, uni[thread].spec_scope.fill),	// yt7pwr
			(char *) CXBbase (buf), CXBsize (buf) * sizeof (COMPLEX));
...
PRIVATE void
do_tx_scope_spectrum (unsigned int thread, CXB buf)			// yt7pwr
{
	memcpy ((char *) &CXBdata (uni[thread].spec_scope.accum, uni[thread].spec_scope.fill),
		(char *) CXBbase (buf), CXBsize (buf) * sizeof (COMPLEX));
	uni[thread].spec_scope.fill = (uni[thread].spec_scope.fill + CXBsize (buf)) & uni[thread].spec_scope.mask;
...
PRIVATE void
do_rx_pre (int k, unsigned int thread)	// changes yt7pwr
{
	...
	correctIQ (rx[thread][k].buf.i, rx[thread][k].iqfix, FALSE, k, rx[thread][k].mode);

	//do_rx_meter (k, rx[thread][k].buf.i, RXMETER_PRE_FILT);
	do_rx_spectrum (k, thread, rx[thread][k].buf.i, SPEC_PRE_FILT);		// yt7pwr
	...
	if (rx[thread][k].mode != SPEC)
	{
		if (rx[thread][k].tick == 0)
		{
			...
			reset_OvSv (rx[thread][k].high_pass_filt.ovsv);
		}

		switch(rx[thread][k].filter_mode)
		{
			case NOTCH:
			{
				filter_OvSv (rx[thread][k].filt.ovsv);					// band pass filter

				for (i = 0; i < n; i++)
				{
					CXBdata(rx[thread][k].high_pass_buf.i,i) = CXBdata(rx[thread][k].buf.o, i);
					CXBdata(rx[thread][k].low_pass_buf.i,i) = CXBdata(rx[thread][k].buf.o, i);
				}

				filter_OvSv (rx[thread][k].high_pass_filt.ovsv);		// high pass filter

				filter_OvSv (rx[thread][k].low_pass_filt.ovsv);			// low pass filter

				for(i=0; i<n; i++)
				{
					CXBdata(rx[thread][k].buf.o,i) = Csub(CXBdata(rx[thread][k].low_pass_buf.o, i),
						CXBdata(rx[thread][k].high_pass_buf.o, i));
				}

				CXBhave( rx[thread][k].high_pass_buf.o) = CXBhave( rx[thread][k].high_pass_buf.i);
				CXBhave( rx[thread][k].low_pass_buf.o) = CXBhave( rx[thread][k].low_pass_buf.i);
			}
			break;
...
do_rx_post (int k, unsigned int thread)
{
	...
	do_rx_scope_spectrum (k, thread, rx[thread][k].buf.o, SPEC_POST_AGC);
	...
	/*if (rx[thread][k].resample.flag == 1)
	{
		rx[thread][k].resample.gen2r->inputArrayLength = n;
		rx[thread][k].resample.gen2i->inputArrayLength = n;
		rx[thread][k].resample.gen2r->input = CXBbase(rx[thread][k].buf.o);
		rx[thread][k].resample.gen2i->input = CXBbase(rx[thread][k].buf.o);
		rx[thread][k].resample.gen2r->output = CXBbase(rx[thread][k].buf.o);
		rx[thread][k].resample.gen2i->output = CXBbase(rx[thread][k].buf.o);
...
/*	if(tx[thread].ctcss.flag && tx[thread].mode == FMN)		// ctcss
	{
		ComplexOSC(tx[thread].ctcss.osc);

		for (i = 0; i < n; i++)
			CXBdata (tx[thread].buf.i, i) = CaddA (CXBdata (tx[thread].buf.i, i), OSCCdata(tx[thread].ctcss.osc, i),
			tx[thread].ctcss.amplitude);
	}*/
...
{
	int i, n = CXBhave (tx[thread].buf.o);
	...
	if (tx[thread].tick == 0)
	{
			...
			reset_OvSv (tx[thread].ctcss_filt.ovsv);
	}

	filter_OvSv (tx[thread].filt.ovsv);		// TX filter
	...
	correctIQ (tx[thread].buf.o, tx[thread].iqfix, TRUE,0, tx[thread].mode);
			...
			for (k = 0; k < uni[thread].multirx.nrx; k++)
			{
				...
				if (uni[thread].multirx.act[k])
				{
					if (!kdone) {
						...
						for (i = 0; i < n; i++)
						{
							CXBimag (rx[thread][k].buf.i, i) = bufl[i];
							CXBreal (rx[thread][k].buf.i, i) = bufr[i];
						}
						CXBhave (rx[thread][k].buf.i) = n;
					} 
					else
					{
						memcpy(rx[thread][k].buf.i,rx[thread][kone].buf.i,CXBhave(rx[thread][kdone].buf.i)*sizeof(COMPLEX));
					}
				}
			}
