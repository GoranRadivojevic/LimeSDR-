		...
		private bool OpenWaveFile(string filename)
		{
            ...
            try
            {
                ...
                if (!CheckSampleRate(fmt.sample_rate) || 
                    (fmt.format == 1 && fmt.sample_rate != Audio.RXSampleRate))
                {
                    reader.Close();
            ...
            try
            {
                ...
                if (checkBoxRecord.Checked && console.chkPower.Checked)
                {
                    ...
                    else
                    {
                        ...
                        file_name += Audio.RXSampleRate.ToString() + " ";
                        file_name += DateTime.Now.ToString() + ".wav";
                        file_name = file_name.Replace("/", "-");
                        file_name = file_name.Replace(":", " ");
                        file = Application.StartupPath + "\\Recordings\\" +
                    ...
                    if (console.CurrentModel == Model.LimeSDR && !Audio.RecordRXPreProcessed)
                        Audio.wave_file_writer = new WaveFileWriter(console.CurrentModel, console.BlockSize2, 2, Audio.SampleRateVAC, file);
                    else
                        Audio.wave_file_writer = new WaveFileWriter(console.CurrentModel, console.RXBlockSize, 2, Audio.RXSampleRate, file);
                }
                else
                {
			...
			ref BinaryReader binread)
		{
            ...
            else if (format == 3)
            {
                ...
                if (sample_rate != Audio.RXSampleRate)
                {
                    ...
                    if (channels > 1) resamp_r = DttSP.NewResamplerF(sample_rate, Audio.RXSampleRate);
                }
            }
